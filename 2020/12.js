const solve = (input) => {
    ...
    const move = (point, vector, steps) => {
        return [point[0] + vector[0] * steps, point[1] + vector[1] * steps];
    };

    const rotate = (vector, direction, degrees) => {
        for (; degrees > 0; degrees -= 90) {
            vector = direction ===  "L" ? [vector[1], -vector[0]] : [-vector[1], vector[0]];
        }
        return vector;
    };
