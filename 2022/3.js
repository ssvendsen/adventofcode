...
const findCommonItemType = (allItems) => {
    const itemCount = allItems.length;
    const aItems = allItems.slice(0, itemCount / 2);
    const bItems = allItems.slice(itemCount / 2);
    const aTypes = new Set(aItems);
    const commonTypes = bItems.filter(itemType => aTypes.has(itemType));
    // assert(commonTypes.length === 1);
...
const priorityFromItemType = (itemType) => {
    const itemCharCode = itemType.charCodeAt(0);
    if (itemCharCode >= charCodes.A && itemCharCode <= charCodes.Z) {
        return itemCharCode - charCodes.A + 27;
    } else {
        return itemCharCode - charCodes.a + 1;
    }
}

const findCommonItemTypeInGroup = (aItems, bItems, cItems) => {
    const aTypes = new Set(aItems);
    const bTypes = new Set(bItems);
    const commonTypes = cItems.filter(itemType => aTypes.has(itemType) && bTypes.has(itemType));
    return commonTypes[0];
}
...
const solve = (input) => {
    ...
    const backpackItems = lines.map(line => line.split(""));

    const commonItemTypes1 = backpackItems.map(items => findCommonItemType(items));
    const priorities1 = commonItemTypes1.map(itemType => priorityFromItemType(itemType));
    const result1 = priorities1.reduce((a, i) => a + i, 0);

    const backpackItemGroups = [];
    while (backpackItems.length) {
        backpackItemGroups.push([
            backpackItems.shift(), backpackItems.shift(), backpackItems.shift()
        ]);
    }
    const commonItemTypes2 = backpackItemGroups.map(group => findCommonItemTypeInGroup(...group));
    const priorities2 = commonItemTypes2.map(itemType => priorityFromItemType(itemType));
    const result2 = priorities2.reduce((a, i) => a + i, 0);
