const solve = (input, skip) => {
    ...
    const getLineValue = (line, fwdRegexp, revRegexp) => {
        const firstMatch = line.match(fwdRegexp)[0];
        const lastMatch = reverse(reverse(line).match(revRegexp)[0]);
        const firstValue = !isNaN(firstMatch) ? +firstMatch : words.indexOf(firstMatch)+1;
        const lastValue = !isNaN(lastMatch) ? +lastMatch : words.indexOf(lastMatch)+1;
        return 10 * firstValue + lastValue;
    }

    const getTotalValue = (lines, strings) => {
        const pattern = strings.join("|");
        const fwdRegexp = new RegExp(pattern);
        const revRegexp = new RegExp(reverse(pattern));
        const lineValues = lines.map(line => getLineValue(line, fwdRegexp, revRegexp));
        return lineValues.reduce((sum, item) => sum + item, 0);
    }

    const result1 = skip === 1 ? undefined : getTotalValue(lines, digits);
    const result2 = skip === 2 ? undefined : getTotalValue(lines, [...digits, ...words]);
...
console.log("Example1", solve(example1, 2)); 
console.log("Example2", solve(example2, 1)); 
