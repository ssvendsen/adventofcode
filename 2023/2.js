const solve = (input) => {
    ...
    const decodeGame = (line) => {
        const [idPart, setsPart] = line.split(": ");
        const id = +idPart.match(/\d+/);
        const setParts = setsPart.split("; ");
        const sets = setParts.map(setPart => {
            const reds = setPart.match(/(\d+) red/)?.[1] ?? 0;
            const greens = setPart.match(/(\d+) green/)?.[1] ?? 0;
            const blues = setPart.match(/(\d+) blue/)?.[1] ?? 0;
            return {red: +reds, green: +greens, blue: +blues};
        });
        return {id, sets};
    }   

    // Task 1: Sum the ID of possible games given there are 
    // only 12 red cubes, 13 green cubes, and 14 blue cubes

    const isPossibleGame = (game, given) => {
        return !game.sets.some(set => set.red > given.red || set.green > given.green || set.blue > given.blue);
    }

    const given = {red: 12, green: 13, blue: 14};
    const games = lines.map(decodeGame);
    const possibleGames = games.filter(game => isPossibleGame(game, given));
    const sumIds = possibleGames.reduce((acc, game) => acc + game.id, 0);

    // Task 2: Sum the 'power' of the minium set of cubes for
    // each game

    const geMinimumSet = (game) => {
        const covering = {red: 0, green: 0, blue: 0};
        for (const set of game.sets) {
            covering.red = Math.max(covering.red, set.red);
            covering.green = Math.max(covering.green, set.green);
            covering.blue = Math.max(covering.blue, set.blue);
        }
        return covering;
    }

    const minimumSets = games.map(geMinimumSet);
    const products = minimumSets.map(({red, green, blue}) => red * green * blue);
    const sumProducts = products.reduce((acc, product) => acc + product, 0);    

    const result1 = sumIds;
    const result2 = sumProducts;
