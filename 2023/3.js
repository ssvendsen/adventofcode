const solve = (input) => {
    ...
    const range = (start, length) => new Array(length).fill(0).map((_, i) => start + i);

    const numberList = [...input.matchAll(/\d+/g)].map(match => (
        {pos: match.index, len: match[0].length, value: +match[0]}
    ));
    const numberMap = new Map(numberList.flatMap(number => (
        range(number.pos, number.len).map(pos => [pos, number]) 
    )));
    //console.log(numberMap);

    const symbolList = [...input.matchAll(/[\-\*\/\#\&\+\@\=\$\%]/g)].map(match => 
        ({pos: match.index, symbol: match[0]}
    ));
    const symbolMap = new Map(symbolList.map(symbol => (
        [symbol.pos, symbol]
    )));
    //console.log(symbolMap);

    const w = input.indexOf("\n") + 1; // width, including newline
    const getAdjacents = (pos) => [-w - 1, -w, -w + 1, -1, 1, w - 1, w, w + 1].map(offset => offset + pos);

    const partNumbers = numberList.filter(({pos, len}) => {
        const numberPositions = range(pos, len);
        return numberPositions.some(pos => (
            getAdjacents(pos).some(adj => (
                symbolMap.has(adj)
            ))
        ));
    });

    const gearRatios = symbolList.map(({pos, symbol}) => {
        if (symbol !== "*") {
            return 0;
        }
        const numbers = new Set();
        getAdjacents(pos).forEach(adj => {
            if (numberMap.has(adj)) {
                numbers.add(numberMap.get(adj));
            }
        });
        if (numbers.size === 2) {
            const [a, b] = numbers.values();
   ...
   const result1 = partNumbers.reduce((acc, number) => acc + number.value, 0);
    const result2 = gearRatios.reduce((acc, ratio) => acc + ratio, 0);
